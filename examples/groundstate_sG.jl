...
optimalBogParameters = [[],
                        ...
                        [],
                        [],
                        [-0.928636025649411, -0.5990816341999012, -0.42357940908377073,
                         -0.3198934246380902, -0.24903438071587236, -0.19302633272158393],
                        [],
                        [],
                        [],
                        [-0.943070404546855, -0.6128324587048812, -0.43774404502240616,
                         -0.3316313967784621, -0.26087264355724327, -0.20946379806428927,
                         -0.17240401270353264, -0.14417356504782697, -0.12194699602552025,
                         -0.10478019491729512],
                        [],
                        [],
                        [],
                        [],
                        [-0.9465800412094174, -0.6146763536373969, -0.438710441446746,
                         -0.3328501526990071, -0.26482206900797045, -0.21652180233527127,
                         -0.1799192563587754, -0.14733025682951933, -0.12527531066573971,
                         -0.10786892144201528, -0.09394935509305521, -0.08248167196995668,
                         -0.07287844663881438, -0.06450819667936558, -0.05806593366498754],
                        [],
                        [],
                        [],
                        [],
                        [-0.9501013782460649, -0.6174497397202185, -0.44098579666337273,
                         -0.33557583359059756, -0.2664771126382963, -0.21756904289495257,
                         -0.18204477839440117, -0.15324594073237924, -0.13089976484642563,
...
#     [-1.0832070648614078, -0.7451083503206792, -0.5594884149655681, -0.4361570000139321], 
#     [-1.112447329438673, -0.7734540934967917, -0.5850305460944181, -0.46218754038059134, -0.3720900965177832], 
...
for (idxB, β) in enumerate(betaList)
    ...
    for (idxL, L) in enumerate(sizeList)
        ...
        elseif initMethod == 1
            ...
            loadFolderStringMPS = "numFiles/" *
                                  modelName *
                                  "/modeOrdering_" *
                                  string(load_modeOrdering) *
                                  "/bogoliubovRot_" *
                                  string(load_bogoliubovR) *
                                  "/truncMethod_" *
                                  string(truncMethod) *
                                  "/MPS/kMax_" *
                                  string(load_kMax)
            ...
            fileStringMPS = @sprintf("%s/mps_kMax_%d_nMax_%d_nMaxZM_%d_L_%0.2f_beta_%0.3f_bondDim_%d_0.jld",
                                     loadFolderStringMPS,
                                     load_kMax,
                                     nMax,
                                     nMaxZM,
        ...
        # construct fileString
        fileStringMPS_0 = @sprintf("%s/mps_kMax_%d_nMax_%d_nMaxZM_%d_L_%0.2f_beta_%0.3f_bondDim_%d_0.jld",
                                   folderStringMPS,
                                   kMax,
                                   nMax,
                                   nMaxZM,
                                   ...
                                   β,
                                   bondDim)
        fileStringMPS_1 = @sprintf("%s/mps_kMax_%d_nMax_%d_nMaxZM_%d_L_%0.2f_beta_%0.3f_bondDim_%d_1.jld",
                                   folderStringMPS,
                                   kMax,
                                   nMax,
                                   nMaxZM,
        ...
        if computeEigenstate_0 == 1
            ...
            if useBasisOptimization_0 == 0
                ...
                labelString = @sprintf("\\beta = %0.1f \\cdot \\beta_{\\textrm{FF}}",
                                       β / βFF)
                labelString = latexstring(labelString)
                plot!(entanglementEntropyPlot,
            ...
            elseif useBasisOptimization_0 == 1
                ...
                # run DMRG for ground state
                groundStateMPS, groundStateEnergy, storeBogoliubovParameters, truncErrors = find_groundstate(initialMPS,
                                                                                                             generate_MPO_sG,
                                                                                                             sG,
                                                                                                             DMRG2BO(;
                                                                                                                     bondDim = bondDim,
        ...
        if computeEigenstate_1 == 1
            if useBasisOptimization_1 == 0
                ...
                excitedStateMPS, excitedStateEnergy = find_excitedstate(initialMPS,
                                                                        hamMPO,
                                                                        lowEnergyStates,
                                                                        DMRG2(;
                                                                              bondDim = bondDim,
            ...
            elseif useBasisOptimization_1 == 1
                ...
                # run DMRG for excited state
                lowEnergyStates = Vector{SparseMPS}([groundStateMPS])
                excitedStateMPS, excitedStateEnergy, storeBogoliubovParameters = find_excitedstate(groundStateMPS,
                                                                                                   generate_MPO_sG,
                                                                                                   lowEnergyStates,
                                                                                                   sG,
                                                                                                   DMRG2BO(;
