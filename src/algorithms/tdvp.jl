...
function perform_timestep!(finiteMPS::SparseMPS, mpoHandle::Function, QFTModel::AbstractQFTModel, timeStep::Union{Float64, ComplexF64}, alg::TDVP2BO)
    """ 2-site TDVP implementation for finiteMPO with global Krylov subspace expansion """
        ...
        if mod(siteIdx, 2) == 0
            ...
            if alg.verbosePrint == 2
                ...
                for (idx, ξ) in enumerate(listOfXiValues)
                    ...
                    storeEntanglementEntropy[idx] = computeRenyiEntropy(applyTwoModeTransformation(sqOp, newAC2));
                    storeAnalyticGradient[idx] = analyticGradientCostFunction(ξ, nMax, kL, kR, PL, PR, newAC2);
                end
            ...
            optimalXi = find_zero(ξ -> analyticGradientCostFunction(ξ, nMax, kL, kR, PL, PR, newAC2), 0.0);
            @show optimalXi
