...
function convert_MPO_33block(vecTensor::SparseMPO)
    ...
    for siteIndex in eachindex(mpo)
        ...
        localVector[1, 3] = TensorMap(zeros, Float64, U1Space(0 => 1) ⊗ physSpace, U1Space(0 => 1) ⊗ physSpace);
        localVector[3, 1] = TensorMap(zeros, Float64, U1Space(0 => 1) ⊗ physSpace, U1Space(0 => 1) ⊗ physSpace);
        for x in [1, 3]
            localVector[x, 2] = TensorMap(zeros, Float64, U1Space(0 => 1) ⊗ physSpace, virtSpaceR ⊗ physSpace);
            localVector[2, x] = TensorMap(zeros, Float64, virtSpaceL ⊗ physSpace, U1Space(0 => 1) ⊗ physSpace);
        end

        # set remaining blocks
        if siteIndex == 1
            localVector[1, 2] = vecTensor[siteIndex];
            localVector[2, 2] = TensorMap(zeros, Float64, virtSpaceL ⊗ physSpace, virtSpaceR ⊗ physSpace);
        elseif siteIndex == L
            localVector[2, 2] = TensorMap(zeros, Float64, virtSpaceL ⊗ physSpace, virtSpaceR ⊗ physSpace);
...
function convert_33block_MPO(vecTensor::Vector{Array{TensorMap}}; leftEnvironment = undef, rightEnvironment = undef)
    ...
    for siteIndex in eachindex(mpoTensors)
        ...
        # compute projectors
        left_projs = generate_projectors([localMatrix[idx, 1].codom[1] for idx in axes(localMatrix, 1)]...)
        right_projs = generate_projectors([localMatrix[1, idy].dom[1] for idy in axes(localMatrix, 2)]...)

        # combine 3x3 block into single mpo tensor
        combined_tensor = TensorMap(zeros, eltype(localMatrix[1, 1]), left_projs[1].codom[1] ⊗ localMatrix[1, 1].codom[2], right_projs[1].codom[1] ⊗ localMatrix[1, 1].dom[2]);
        for idx in axes(localMatrix, 1), idy in axes(localMatrix, 2)
            @tensor temp[-1 -2; -3 -4] := left_projs[idx][-1, 1] * localMatrix[idx, idy][1, -2, 3, -4] * right_projs[idy]'[3, -3];
    ...
    if leftEnvironment == undef
        ...
        leftEnvironment[1] = TensorMap(ones, eltype(vecTensor[1][2, 2]), U1Space(0 => 1), rightspace);
        for idx in 2 : length(leftEnvironment)
            rightspace = vecTensor[1][idx, 1].codom[1];
            leftEnvironment[idx] = TensorMap(zeros, eltype(vecTensor[1][2, 2]), U1Space(0 => 1), rightspace);
        end
    end
    right_projs = generate_projectors([leftEnvironment[idx].dom[1] for idx in eachindex(leftEnvironment)]...);
    new_left = TensorMap(zeros, eltype(leftEnvironment[1]), leftEnvironment[1].codom[1], right_projs[1].codom[1]);
    ...
    if rightEnvironment == undef
        ...
        for idy in 1 : (length(rightEnvironment) - 1)
            ...
            rightEnvironment[idy] = TensorMap(zeros, eltype(vecTensor[end][2, 2]), leftSpace, U1Space(0 => 1));
        end
        leftSpace = vecTensor[end][1, end].dom[1];
        rightEnvironment[end] = TensorMap(ones, eltype(vecTensor[end][2, 2]), leftSpace, U1Space(0 => 1));
    end
    left_projs = generate_projectors([rightEnvironment[x].codom[1] for x in eachindex(rightEnvironment)]...)
    new_right = TensorMap(zeros, eltype(rightEnvironment[1]), left_projs[1].codom[1], rightEnvironment[1].dom[1])
...
function generate_projectors(V1::GradedSpace, V2::GradedSpace...)
    Vjoined = ⊕(V1, V2...)
    proj1 = TensorMap(zeros, Float64, Vjoined, V1)
    shifts = Dict()
    ...
    for (j, V) in enumerate(V2)
        proj2 = TensorMap(zeros, Float64, Vjoined, V)
...
"""
function left_canonicalize(matrixMPO; rightEnvironment = undef, truncation::Bool = false, truncError::Float64 = 1e-14, tol::Float64 = 1e-12, verbose::Int64 = 0)
    ...
    if rightEnvironment == undef
        ...
        rightEnvironment[1] = TensorMap(zeros, eltype(matMPO[end][2, 2]),  leftSpace, U1Space(0 => 1));
        leftSpace = matMPO[end][1, 2].dom[1];
        rightEnvironment[2] = TensorMap(zeros, eltype(matMPO[end][2, 2]),  leftSpace, U1Space(0 => 1));
        leftSpace = matMPO[end][1, 3].dom[1];
        rightEnvironment[3] = TensorMap(ones, eltype(matMPO[end][2, 2]),  leftSpace, U1Space(0 => 1));
    else
    ...
    for siteIndex in 1 : L
        ...
        @tensor fused_tensor[-1 -2; -3 -4] := proj_at[-1, 1] * at[1, -2, -3, -4] + proj_W[-1, 1] * W[1, -2, -3, -4];
        ...
        matMPO[siteIndex][3, 2] = TensorMap(zeros, eltype(matMPO[siteIndex][3, 2]), U1Space(0 => 1) ⊗ localSpace, newRightSpace ⊗ localSpace);
        ...
        if siteIndex != L
            ...
            # a bit of bookeeping
            matMPO[newSiteIndex][2, 1] = TensorMap(zeros, eltype(matMPO[newSiteIndex][2, 1]), R.codom[1] ⊗ localSpace, U1Space(0 => 1) ⊗ localSpace);
...
"""
    ...
    - verbose -> control the verbosity of the function
"""
function right_canonicalize(matrixMPO; leftEnvironment = undef, truncation = false, truncError::Float64 = 1e-14, tol::Float64 = 1e-12, verbose::Int64 = 0)
    ...
    if leftEnvironment == undef
        ...
        leftEnvironment[1] = TensorMap(ones, eltype(matMPO[1][2, 2]),  U1Space(0 => 1), rightSpace);
        rightSpace = matMPO[1][2, 1].codom[1];
        leftEnvironment[2] = TensorMap(zeros, eltype(matMPO[1][2, 2]), U1Space(0 => 1), rightSpace);
        rightSpace = matMPO[1][3, 1].codom[1];
        leftEnvironment[3] = TensorMap(zeros, eltype(matMPO[1][2, 2]), U1Space(0 => 1), rightSpace);
    else
    ...
    for siteIndex in reverse(1 : L)
        ...
        matMPO[siteIndex][2, 2] = Wf;
        matMPO[siteIndex][2, 1] = TensorMap(zeros, eltype(matMPO[siteIndex][2, 1]), newLeftSpace ⊗ localSpace, U1Space(0 => 1) ⊗ localSpace);
        ...
        if siteIndex != 1
            ...
            # a bit of bookeeping
            matMPO[newSiteIndex][3, 2] = TensorMap(zeros, eltype(matMPO[newSiteIndex][3, 2]), U1Space(0 => 1) ⊗ localSpace, R.dom[1] ⊗ localSpace);
