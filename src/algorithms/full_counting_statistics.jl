...
function compute_phase_distribution(mS::MassiveSchwingerModel, finiteMPS::SparseMPS; ϕMax::Float64 = 4π, λNums::Int64 = 101, verbosePrint::Bool = false)
    ...
    for idxL in eachindex(lambdaValues)
        ...
        for (siteIdx, momentumVal) in enumerate(momentumModes)
            ...
            if momentumVal == 0
                localOperators[siteIdx] = localVertexOp_mS(momentumVal, physSpace, λ, M, L, bogoliubovR);
            else
                localOperators[siteIdx] = exp(-(λ/R)^2 / (2 * abs(momentumVal))) * localVertexOp_mS(momentumVal, physSpace, λ, M, L, bogoliubovR);
            end
        end
