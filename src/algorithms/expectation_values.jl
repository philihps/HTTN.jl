...
function variance_mpo(finiteMPS::SparseMPS, finiteMPO::SparseMPO)
    ...
    # compute H|ψ⟩
    finiteMPOMPS = applyMPO(finiteMPO, myMPS; truncErr = 1.0e-4, compressionAlg = "zipUp")
...
function expectation_values_density_matrix(
        thermalDensityMatrix::SparseMPO,
        finiteMPO::SparseMPO
    )
    ...
    # contract from left to right
    boundaryL = ones(
        space(thermalDensityMatrix[1], 1),
    ...
    boundaryR = ones(
        space(thermalDensityMatrix[N], 3)' ⊗ space(finiteMPO[N], 3)',
