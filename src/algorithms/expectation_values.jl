...
function expectation_values_density_matrix(thermalDensityMatrix::SparseMPO, finiteMPO::SparseMPO)
    ...
    # contract from left to right
    boundaryL = ones(space(thermalDensityMatrix[1], 1), space(finiteMPO[1], 1) ⊗ space(thermalDensityMatrix[1], 1))
    for siteIdx = 1 : +1 : N
        @tensor boundaryL[-1; -2 -3] := boundaryL[1, 3, 5] * thermalDensityMatrix[siteIdx][5, 4, -3, 6] * finiteMPO[siteIdx][3, 2, -2, 4] * conj(thermalDensityMatrix[siteIdx][1, 2, -1, 6])
    end
    boundaryR = ones(space(thermalDensityMatrix[N], 3)' ⊗ space(finiteMPO[N], 3)', space(thermalDensityMatrix[N], 3)')
