...
@kwdef struct DMRG2
    ...
    convTolE::Float64 = 1.0e-6
    eigsTol::Float64 = 1.0e-8
...
function find_groundstate!(finiteMPS::SparseMPS, finiteMPO::SparseMPO, alg::DMRG2)
    ...
    while runOptimization
        ...
        while runOptimizationDMRG
            ...
            alg.verbosePrint > 0 &&
                @printf(
                "DMRG step %d - energy, convergence = %0.8f, %0.8e\n",
                loopCounter,
                mpoExpVal,
                energyConvergence
            )
...
function find_groundstate!(
    ...
    while runOptimization
        ...
        while runOptimizationDMRG
            ...
            for siteIdx in 1:+1:(length(finiteMPS) - 1)
                ...
                if mod(siteIdx, 2) == 0 && loopCounter > alg.startOptimization
                    ...
                    if any(abs.(optimalXi) .> 1.0e-3)
                        ...
                        if checkAcceptance(
                                costFuncPre, costFuncPost, bogParameters[kR + 1],
                                optimalXi
                            )
            ...
            # store bogParameters
            storeBogoliubovParameters = vcat(
...
function find_excitedstate!(
        finiteMPS::SparseMPS, finiteMPO::SparseMPO,
    ...
    # set penaltyWeights
    penaltyWeights = 1.0e2 * ones(length(previoMPS))
        ...
        while runOptimizationDMRG
            ...
            for siteIdx in 1:+1:(length(finiteMPS) - 1)
                ...
                for orthMPO in eachindex(previoMPS)
                    thetasO[orthMPO] = permute(
                        previoMPS[orthMPO][siteIdx] *
                            permute(
                            previoMPS[orthMPO][siteIdx + 1],
                            ((1,), (2, 3))
                        ),
                        (
                            (1, 2),
                ...
                for orthIdx in eachindex(previoMPS)
                    (Q, R) = leftorth(
                        previoMPS[orthIdx][siteIdx], ((1, 2), (3,));
                        alg = QRpos()
                    )
                    previoMPS[orthIdx][siteIdx + 0] = permute(Q, ((1, 2), (3,)))
                    previoMPS[orthIdx][siteIdx + 1] = permute(
                        R *
                            permute(
                ...
                # update projEnvsL
                for orthIdx in eachindex(previoMPS)
                    projEnvsL[orthIdx][siteIdx + 1] = update_MPSEnvL(
                        projEnvsL[orthIdx][siteIdx],
                        finiteMPS[siteIdx],
            ...
            for siteIdx in (length(finiteMPS) - 1):-1:1
                ...
                for orthIdx in eachindex(previoMPS)
                    (L, Q) = rightorth(
                        previoMPS[orthIdx][siteIdx + 1], ((1,), (2, 3));
                        alg = LQpos()
                    )
                    previoMPS[orthIdx][siteIdx + 0] = permute(
                        permute(
                            previoMPS[orthIdx][siteIdx + 0],
                            ((1, 2), (3,))
                        ) * L,
                        (
                            (1, 2),
                ...
                # update projEnvsR
                for orthIdx in eachindex(previoMPS)
                    projEnvsR[orthIdx][siteIdx + 0] = update_MPSEnvR(
                        projEnvsR[orthIdx][siteIdx + 1],
                        finiteMPS[siteIdx + 1],
                        previoMPS[orthIdx][siteIdx + 1]
                    )
                end
                ...
                if mod(siteIdx, 2) == 0 && loopCounter >= alg.startOptimization
                    ...
                    if alg.verbosePrint == 2
                        ...
                        for (idx, ξ) in enumerate(listOfXiValues)
                            ...
                            storeAnalyticGradient[idx] = analyticGradientCostFunction(
                                ξ,
                                nMax,
                                kL,
                                kR,
                    ...
                    optimalXi = find_zero(
                        ξ -> analyticGradientCostFunction(
                            ξ, nMax, kL, kR,
                            PL, PR,
                            newTheta
                        ),
