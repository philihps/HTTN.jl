...
function initializeMPOEnvironments(
    ...
    mpoEnvL[1] = ones(
        ComplexF64, space(finiteMPS[1], 1),
    ...
    for siteIdx in N:-1:(centerPos + 1)
        mpoEnvR[siteIdx - 1] = update_MPOEnvR(
            mpoEnvR[siteIdx], finiteMPS[siteIdx],
            finiteMPO[siteIdx], finiteMPS[siteIdx]
        )
    end
...
function initializePROEnvironments(
        ketMPS::SparseMPS, orthStates::Vector{SparseMPS};
        centerPos::Int64 = 1
    )
    ...
    for orthIdx in eachindex(orthStates)
        ...
        for siteIdx in 1:+1:(centerPos - 1)
            projEnvL[siteIdx + 1] = update_MPSEnvL(
                projEnvL[siteIdx], ketMPS[siteIdx],
                braMPS[siteIdx]
            )
        end
        ...
        for siteIdx in N:-1:2
            projEnvR[siteIdx - 1] = update_MPSEnvR(
                projEnvR[siteIdx], ketMPS[siteIdx],
                braMPS[siteIdx]
            )
        end
