...
function dotMPO(MPOA::SparseMPO, MPOB::SparseMPO)
    ...
    # get length of MPOs
    N = length(MPOA)
    N != length(MPOB) && throw(DimensionMismatch("lengths of MPO A ($N) and MPO B ($length(MPOB)) do not match"))

    # initialize overlap
    overlapMPO = ones(space(MPOA[1], 1), space(MPOB[1], 1))
    for siteIdx = 1 : N
        @tensor overlapMPO[-1; -2] := overlapMPO[1, 2] * conj(MPOA[siteIdx][1, 3, -1, 4]) * MPOB[siteIdx][2, 3, -2, 4]
    end
...
function multiplyMPOs(MPOA::SparseMPO, MPOB::SparseMPO;
    ...
    if compressionAlg == "densityMatrix"
        ...
        mpoEnvL[1] = ones(space(compressedMPO[1], 1) ⊗ space(MPOB[1], 1), space(MPOB[1], 1) ⊗ space(compressedMPO[1], 1));
        mpoEnvR[N] = ones(space(compressedMPO[N], 3)' ⊗ space(MPOB[N], 3)', oneunit(spacetype(compressedMPO[N])));

        # compute mpoEnvL
        for siteIdx = 1 : (N - 1)
            @tensor mpoEnvL[siteIdx + 1][-1 -2; -3 -4] := mpoEnvL[siteIdx][1, 2, 4, 6] * conj(compressedMPO[siteIdx][1, 3, -1, 8]) * conj(MPOB[siteIdx][2, 5, -2, 3]) * MPOB[siteIdx][4, 5, -3, 7] * compressedMPO[siteIdx][6, 7, -4, 8];
        end
        ...
        for siteIdx = N : -1 : 1
            ...
            # construct density matrix
            @tensor rho[-1 -2 -3; -4 -5 -6] := mpoEnvL[siteIdx][1, 2, 4, 5] * conj(compressedMPO[siteIdx][1, 3, 9, -3]) * conj(MPOB[siteIdx][2, -1, 10, 3]) * MPOB[siteIdx][4, -4, 8, 6] * compressedMPO[siteIdx][5, 6, 7, -6] * conj(mpoEnvR[siteIdx][9, 10, -2]) * mpoEnvR[siteIdx][7, 8, -5];
            rho /= tr(rho);
            ...
            # compute right environment for the site to the left
            if siteIdx > 1
                @tensor mpoEnvR[siteIdx - 1][-1 -2; -3] := compressedMPO[siteIdx][-1, 3, 1, 6] * MPOB[siteIdx][-2, 4, 2, 3] * mpoEnvR[siteIdx][1, 2, 5] * U[4, 5, 6, -3];
            end
    ...
    elseif compressionAlg == "zipUp"
        ...
        for siteIdx = 1 : N
            ...
            if siteIdx < N
                @tensor localTensor[-1 -2 -3; -4 -5] := isomoL[-2, 1, 3] * compressedMPO[siteIdx][1, 2, -4, -1] * MPOB[siteIdx][3, -3, -5, 2];
                U, S, V = tsvd(localTensor, (1, 2, 3), (4, 5), trunc = truncdim(maxDim) & truncerr(truncErr), alg = TensorKit.SVD());
                compressedMPO[siteIdx] = permute(U, (2, 3), (4, 1));
                isomoL = S * V;
            else
                @tensor compressedMPO[siteIdx][-1 -2; -3 -4] := isomoL[-1, 1, 3] * compressedMPO[siteIdx][1, 2, 4, -4] * MPOB[siteIdx][3, -2, 5, 2] * isomoR[4, 5, -3];
            end
    ...
    elseif compressionAlg == "variationalContraction"
        ...
        mpoEnvL[1] = ones(ComplexF64, space(compressedMPO[1], 1), space(MPOB[1], 1) ⊗ space(MPOA[1], 1));
        mpoEnvR[N] = ones(ComplexF64, space(MPOA[N], 3)' ⊗ space(MPOB[N], 3)', space(compressedMPO[N], 3)');

        # compute mpoEnvR, since the MPS is in right-canonical form
        for siteIdx = N : -1 : 2
            mpoEnvR[siteIdx - 1] = update_MPOMPOEnvR(mpoEnvR[siteIdx], MPOA[siteIdx], MPOB[siteIdx], compressedMPO[siteIdx]);
        end
        ...
        while runVariationalSweep
            ...
            for siteIdx = 1 : +1 : (N - 1)
                ...
                # update mpoEnvL
                mpoEnvL[siteIdx + 1] = update_MPOMPOEnvL(mpoEnvL[siteIdx], MPOA[siteIdx], MPOB[siteIdx], compressedMPO[siteIdx]);
            ...
            for siteIdx = (N - 1) : -1 : 1
                ...
                # update mpoEnvR
                mpoEnvR[siteIdx + 0] = update_MPOMPOEnvR(mpoEnvR[siteIdx + 1], MPOA[siteIdx + 1], MPOB[siteIdx + 1], compressedMPO[siteIdx + 1]);
